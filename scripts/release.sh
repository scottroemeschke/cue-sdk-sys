...
CARGO_VERSION=$(grep '^version' cue-sdk-sys/Cargo.toml | head -1 | sed 's/.*"\(.*\)".*/\1/')
if [[ "$CARGO_VERSION" != "$VERSION" ]]; then
  echo "Error: Cargo.toml version is $CARGO_VERSION but releasing $VERSION." >&2
  echo "Update cue-sdk-sys/Cargo.toml first." >&2
  exit 1
fi
...
echo "Track progress: https://github.com/scottroemeschke/cue-sdk-sys/actions"
